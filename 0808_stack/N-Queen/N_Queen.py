...
def place_queen(r, n):
    ...
    # 2. 재귀 호출
        ...
        # 내가 r번째 행에 퀸을 놓지 않았다면 => 다음 행 진행 불가(답 없음)
    ...
    for c in range(N):
        ...
        # (r,c) 위를 검사해서 퀸을 공격할 수 있는 위치에 있다면 False 로 바꿈
        ...
        for i in range(1, r + 1):
            if r - i >= 0 and c - i >= 0 and board[r-i][c-i] == 1:
                ...
                break
        # 우상 대각선 검사
            if r - i >= 0 and N > c + i >= 0 and board[r-i][c+i] == 1:
                can_place = False
                break
        # 위쪽 검사 끝나고 다음 행으로 갈 수 있으면 => 재귀 호출
        if can_place:
            # r+1 번째 행으로 퀸을 놓으러 가자
            board[r][c] = 1 # 현재 위치에 퀸을 놓았다고 표시
...
for tc in range(1, T+1):
    # NxN 크기의 체스판에 N개의 퀸을 서로 공격하지 않는 위치에 놓기
    N = int(input())
